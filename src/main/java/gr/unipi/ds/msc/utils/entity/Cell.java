...
public class Cell {
	...
	public List<Tuple2<Long, Double>> getNeighborWeightedAttributeValuesList() {
        ...
        for (long i = x - neighborDistance; i <= x + neighborDistance; i++) {//aTHS
            if ((i < 0) || (i >= params.longitudeSegments))
                continue;
            for (long j = y - neighborDistance; j <= y + neighborDistance; j++) {//aTHS
                if ((j < 0) || (j >= params.latitudeSegments))
                    continue;
                for (long k = t - neighborDistance; k <= t + neighborDistance; k++) {//aTHS
                    if ((k < 0) || (k >= params.dateSegments))
                        continue;
                    //cell [i,j,k] might not exists based on trajectory points!!!
                    double power = Math.max(Math.max(Math.abs(x-i), Math.abs(j-y)), Math.abs(k-t));
                    if (power > 0) {//if power == 0 then ???? it should give power=1 not 0
                        power = power - 1;
                    }
