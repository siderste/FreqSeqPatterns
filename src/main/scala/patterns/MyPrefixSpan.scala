...
/**
  ...
  */
//@Since("1.5.0")
class MyPrefixSpan private (
                             private var minSupport: Double,
                             private var maxPatternLength: Int,
                             private var maxLocalProjDBSize: Long,
                             private var maxDToFPatterns: Int) extends Logging with Serializable {

  import MyPrefixSpan._
  ...
  /**
    ...
    */
  //@Since("1.5.0")
  //def this() = this(0.1, 10, 50L)
  def this() = this(0.1, 10, 32000000L, Int.MaxValue)
  ...
  /**
    * Sets the max temporal duration between two valid patterns
    * @param maxDTofPatterns
    */
  def setMaxDTofPatterns(maxDTofPatterns: Int) = {
    require(maxDTofPatterns >= Int.MinValue && maxDTofPatterns <= Int.MaxValue,
      s"The maximum temporal duration value must be in ["+Int.MinValue+", "+Int.MaxValue+"], but got $maxDTofPatterns.")
    this.maxDToFPatterns = maxDTofPatterns
    this
  }
  ...
  def run(dataset: Dataset[ (Long, Array[Array[Item]]) ], spark: SparkSession):  RDD[((Array[Array[Int]], Int), scala.collection.Map[Long, Array[Vector]])]
  //def run(dataset: DataFrame, spark: SparkSession) : DataFrame
  = {

    val data = dataset.rdd
    ...
    // Find initial frequent items
    val freqItems = findFrequentItems(data, minCount) // Array of IDs of frequent Items
    ...
    val itemToInt = freqItems.toIterator.map(freqitem=> freqitem._1).zipWithIndex.toMap //List((2,0), (3,1), (1,2))
    ...
    def toPublicRepr(pattern: Array[Int]): Array[Array[Int]] = {
      ...
      val itemsetBuilder = mutable.ArrayBuilder.make[Int]
      ...
      while (i < n) {
        ...
        if (x == 0) {
          ...
          itemsetBuilder += x // using 1-indexing in internal format
        }
        i += 1
      }
    ...
    val freqSequences = results
      ...
      .map { case (seq: Prefix, count: Long) =>
        ...
        val m = seq.instances.mapValues(setItems=>{
          setItems.toArray.map(_.toSpatialDenseVector)
        })

        ( (sequence,sequence.length), m )
...
//@Since("1.5.0")
object MyPrefixSpan extends Logging {
  ...
  /**
    ...
    */
  private[patterns] def findFrequentItems(
                                      data: RDD[ (Long, Array[Array[Item]]) ],
                                      minCount: Long): Array[(Int, Long)] = {
    ...
    data.flatMap { itemsets =>
      ...
      itemsets._2.foreach ( itemset => itemset.foreach (item => uniqItems += item.area_id ) ) // Item is based on its area_id only
  ...
  /**
    ...
    */
  private[patterns] def toDatabaseInternalRepr(
                                           data: RDD[(Long, Array[Array[Item]]) ],
                                           itemToInt: Map[Int, Int]): RDD[(Long, Array[Item]) ] = {
    ...
    data.flatMap { itemsets =>
      ...
      if (containsFreqItems) {
        Iterator.single(itemsets._1, allItems.result())
      } else {
        Iterator.empty
      }
    }
  ...
  /**
    ...
    * @see [[Postfix]]
    */
  private[patterns] def genFreqPatterns(
                                         data: RDD[ (Long, Array[Item]) ],
                                         initialPrefixes: Array[(Int, Long)], //myversion, not used yet
                                         minCount: Long,
    ...
    var postfixes= data.map{items =>
      new Postfix(items._2, maxDTofPatterns,0, Array.empty,-1,items._1)
    }

    // Local frequent patterns (prefixes) and their counts and their transitions
    val localFreqPatterns = mutable.ArrayBuffer.empty[(Prefix, Long)]
    ...
    while (largePrefixes.nonEmpty) {
      ...
      //TODO costly????
      val myfreqPrefixes = postfixes.flatMap { postfix => {  // + 7.221 s
      ...
      myfreqPrefixes.foreach { case ((id, item), (count, projDBSize, instances)) =>
        ...
        if (newPrefix.length < maxPatternLength) {
          if (projDBSize > maxLocalProjDBSize) {
            //TODO keep maxLocalProjDBSize LARGE ENOUGH to have only one loop in master
            // cause the flatmap of postfix.projectFull(prefix) looses the recursive nature of prefixspan
            newLargePrefixes += newPrefix.id -> newPrefix //no 0 at the end
          } else {
            smallPrefixes += newPrefix.id -> newPrefix //no 0 at the end
          }
        }
      }
    ...
    if (numSmallPrefixes > 0) {
      ...
      val distributedFreqPattern = postfixes.flatMap { postfix =>  //  + 10.914 s
        ...
        val localPrefixSpan = new MyLocalPrefixSpan(minCount, maxPatternLength - prefix.length)
  ...
  private[patterns] class Prefix(val items: Array[Int], val length: Int, val instances: mutable.Map[Long, Set[Item]])
    ...
    /** Current id for this prefix. */
    val currId: Int = Prefix.currId
        ...
        newInstances.keys.foreach(newTraj=>{
          if (instances.contains(newTraj)){ //expected to be always true
            ...
            while (newInstancesItems.nonEmpty){
              ...
              while(currInstancesTimes.nonEmpty && newInstancesItems.nonEmpty) {
                var currInstancesTimesArray = currInstancesTimes.take(currLength)
                var bestInstnacesTimes = currInstancesTimesArray
                ...
                if (bestInstnacesTimes.size > 0) { //found candidates
                  var best = bestInstnacesTimes.max
                  if (!newSetInstances.contains(best)) {
                    newSetInstances = newSetInstances ++ currInstancesTimesArray.toSet ++ newInstancesTimesArray.toSet //add nearest
                  }
                  newInstancesItems = newInstancesItems -- newInstancesTimesArray //remove found times
                  if (newInstancesItems.nonEmpty){
                    newInstancesTimesArray = newInstancesItems.take(newLength)
              ...
              newInstancesItems = newInstancesItems -- newInstancesTimesArray //remove checked times
            }//next newInstancesItems batch
            //if (newSetInstances.nonEmpty)
              result.put(newTraj,newSetInstances)
          }
        })
      }else{
        result = newInstances
      }
      //log.warn("m result="+result.mkString("[",",","]"))
      result
    }
    ...
    def :+(item: Int, newInstances: mutable.Map[Long,  Set[Item]], maxDTofPatterns: Int): Prefix = {
      ...
      var mergedInstances = mergeTimely(newInstances, length, 1, maxDTofPatterns)
      if (item < 0) {
        new Prefix(items :+ -item, length + 1, mergedInstances)
      } else {
        new Prefix(items ++ Array(0, item), length + 1, mergedInstances)
      }
    }

    /** Closes this prefix. */
    def close(): Prefix = {
      new Prefix(items :+ 0, length, instances)
    }

    /** Append a distributed pattern found to this prefix. */
    def ++(pattern: MyLocalPrefixSpan.ReversedPrefix, maxDTofPatterns: Int): Prefix = {
      var patternLength = pattern.items.filter(p=>p!=0).size
      //var log = LogManager.getRootLogger
      //log.warn("m prefix="+items.mkString("[",",","]")+", new pattern="+pattern.toSequence.mkString("[",",","]") )
      var mergedInstances = mergeTimely(pattern.instances, length, patternLength, maxDTofPatterns)
      new Prefix(items++pattern.toSequence, length+patternLength, mergedInstances)
    }
  }

  private[patterns] object Prefix {
    ...
    private def currId: Int = counter.intValue()

    /** An empty [[Prefix]] instance. */
    val empty: Prefix = new Prefix(Array.empty, 0, mutable.Map.empty)
  }
  ...
  /**
    ...
    */
  private[patterns] class Postfix(
                                   val items: Array[Item],
                                   val maxDTofPatterns: Int = Int.MaxValue,
                                   val start: Int = 0,
    ...
    def project(prefix: Int): Postfix = {
      ...
      new Postfix(items, maxDTofPatterns, newStart, newPartialStarts.result(), items(newStart).time, sequenceId)
    }
    ...
    private def projectFull(prefix: Array[Int]): Array[Postfix] = {
      val postfixes = mutable.ArrayBuilder.make[Postfix]
      var partial = true
      var cur = this
      var newPostfix = this
      if ( prefix.length > 0 ) {
        var i = 0
        val np = prefix.length
        while (i < np && newPostfix.nonEmpty) {
          val x = prefix(i)
          if (x == 0) {
            partial = false
          } else {
            if (partial) {
              newPostfix = newPostfix.project(-x)
            } else {
              newPostfix = newPostfix.project(x)
              partial = true
            }
          }
          if ( i+1 >= np && newPostfix.nonEmpty ){ //if prefix items are checked and newpostfix not empty
            postfixes += new Postfix(newPostfix.items, maxDTofPatterns, newPostfix.start,
              newPostfix.partialStarts, newPostfix.prevTimestamp, newPostfix.sequenceId)//to avoid override prevtime
            newPostfix.prevTimestamp = cur.prevTimestamp//reset prevTime if nonEmpty for next loop with new prefix
            i = 0 //to rescan newPostfix from newStart with all prefix items => full projection
          }else{
            i += 1 // get next prefix item to check
          }
        }
      }else{
        postfixes += new Postfix(newPostfix.items, maxDTofPatterns, newPostfix.start, newPostfix.partialStarts, newPostfix.prevTimestamp, newPostfix.sequenceId)
      }
      postfixes.result()
    }

    private def projectFull2(prefix: Array[Int]): Array[Postfix] = {
      val postfixes = mutable.ArrayBuilder.make[Postfix]
      var firstprefixitem = prefix.filter(p=>p!=0).head
      ...
      var cur = this
      var newPostfix = this
      if ( prefix.length > 0 && postfixplaces.size > 0) {
        var p = 0
        var i = 0
        val np = prefix.length
        while (i < np && newPostfix.nonEmpty) {
          ...
          if (x == 0) {
            ...
            if (partial) {
              ...
              newPostfix = newPostfix.project(x)
              if (newPostfix.nonEmpty) {// non empty newPostfix
                if (i+1 >= np){//prefix checked
                  postfixes += new Postfix(newPostfix.items, maxDTofPatterns, newPostfix.start, newPostfix.partialStarts, newPostfix.prevTimestamp, newPostfix.sequenceId)//to avoid override prevtime
                  newPostfix.prevTimestamp = cur.prevTimestamp//reset prevTime if nonEmpty for next loop with new prefix
                  while (p<postfixplaces.size-1 && newPostfix.start > postfixplaces(p)) p+=1
                  i = 0//to rescan newPostfix from newStart with all prefix items => full projection
                }else{
                  i+=1
                }
              }else{// empty newPostfix
                if (i+1 >= np) { //prefix checked
                  postfixes += new Postfix(newPostfix.items, maxDTofPatterns, newPostfix.start, newPostfix.partialStarts, newPostfix.prevTimestamp, newPostfix.sequenceId)
                  if (p<postfixplaces.size-1){//can restart projection on next place minus 1 to point to 0
                    p+=1
                    newPostfix = new Postfix(this.items, maxDTofPatterns, postfixplaces(p)-1, this.partialStarts, cur.prevTimestamp, this.sequenceId)
                    i=0
                  }else{
                    //skip to exit while
                  }
    ...
    def projectFull(prefix: Prefix): Array[Postfix] = projectFull(prefix.items)

    def projectFull2(prefix: Prefix): Array[Postfix] = projectFull2(prefix.items)
    ...
    def compressed: Postfix = {
      if (start > 0) {
        new Postfix(items.slice(start, items.length), maxDTofPatterns, 0, partialStarts.map(_ - start), items(start).time, sequenceId)
      } else {
        this
      }
    }
  ...
  /**
    ...
    */
  //@Since("1.5.0")
  class FreqSequence //@Since("1.5.0")
  ( //@Since("1.5.0")
    val sequence: Array[Array[Int]],
    //@Since("1.5.0")
    val freq: Long,
    val instances: mutable.Map[Long, Set[Long]]) extends Serializable {
    /**
      * Returns sequence as a Java List of lists for Java users.
      */
    //@Since("1.5.0")
    def javaSequence: ju.List[ju.List[Int]] = sequence.map(_.toList.asJava).toList.asJava
  }
}
...
/**
  ...
  */
//@Since("1.5.0")
class MyPrefixSpanModel //@Since("1.5.0")
(
    //@Since("1.5.0")
    val freqSequences: RDD[MyPrefixSpan.FreqSequence])
  ...
  override def save(sc: SparkContext, path: String): Unit = {
    MyPrefixSpanModel.SaveLoadV1_0.save(this, path)
  }
...
//@Since("2.0.0")
object MyPrefixSpanModel extends Loader[MyPrefixSpanModel] {

  //@Since("2.0.0")
  override def load(sc: SparkContext, path: String): MyPrefixSpanModel = {
    MyPrefixSpanModel.SaveLoadV1_0.load(sc, path)
  }

  private[patterns] object SaveLoadV1_0 {

    private val thisFormatVersion = "1.0"

    private val thisClassName = "org.apache.spark.mllib.fpm.PrefixSpanModel"

    def save(model: MyPrefixSpanModel, path: String): Unit = {
    ...
    def load(sc: SparkContext, path: String): MyPrefixSpanModel = {
    ...
    def loadImpl(freqSequences: DataFrame, sample: Any): MyPrefixSpanModel = {
      val freqSequencesRDD = freqSequences.select("sequence", "freq").rdd.map { x =>
        val sequence = x.getAs[Seq[Seq[Int]]](0).map(_.toArray).toArray
        val freq = x.getLong(1)
        new MyPrefixSpan.FreqSequence(sequence, freq, mutable.Map.empty)
      }
      new MyPrefixSpanModel(freqSequencesRDD)
    }
  }
}
