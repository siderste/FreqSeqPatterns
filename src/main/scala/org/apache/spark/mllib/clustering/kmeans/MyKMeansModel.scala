...
class MyKMeansModel(@Since("1.0.0") val clusterCenters: Array[Vector],
                    @Since("2.4.0") val distanceMeasure: String,
                    @Since("2.4.0") val trainingCost: Double,
                    private[spark] val numIter: Int)
  extends Saveable with Serializable with PMMLExportable {

  private val distanceMeasureInstance: MyDistanceMeasure =
    MyDistanceMeasure.decodeFromString(distanceMeasure).asInstanceOf[MyDistanceMeasure]
  ...
  def this(clusterCenters: Array[Vector]) =
    this(clusterCenters: Array[Vector], MyDistanceMeasure.RV, 0.0, -1)
  ...
  override def save(sc: SparkContext, path: String): Unit = {
    MyKMeansModel.SaveLoadV2_0.save(sc, this, path)
  }
...
@Since("1.4.0")
object MyKMeansModel extends Loader[MyKMeansModel] {

  @Since("1.4.0")
  override def load(sc: SparkContext, path: String): MyKMeansModel = {
  ...
  private[clustering] object SaveLoadV1_0 {
    ...
    def save(sc: SparkContext, model: MyKMeansModel, path: String): Unit = {
    ...
    def load(sc: SparkContext, path: String): MyKMeansModel = {
      ...
      new MyKMeansModel(localCentroids.sortBy(_.id).map(_.point))
    }
  }
  ...
  private[clustering] object SaveLoadV2_0 {
    ...
    def load(sc: SparkContext, path: String): MyKMeansModel = {
      ...
      new MyKMeansModel(localCentroids.sortBy(_.id).map(_.point), distanceMeasure, trainingCost, -1)
    }
  }
}