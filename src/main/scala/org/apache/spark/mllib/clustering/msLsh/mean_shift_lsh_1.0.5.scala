...
class MsLsh private (
  ...
  def labelizing(sc:SparkContext, rdd: RDD[(String,Vector,Vector)], maxMinArray:Array[Array[Double]]) : Mean_shift_lsh_model = {
    ...
    */
    val rdd_Ystar_labeled = clusterByLshBucket.map{ case(clusterID, (idx, mod, originalVector)) =>
      (clusterID, (idx, originalVector, mod))}.partitionBy(new spark.HashPartitioner(numPart)).cache
