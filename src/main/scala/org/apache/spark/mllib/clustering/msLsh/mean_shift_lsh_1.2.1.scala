...
import org.apache.spark.mllib.clustering.msLsh.Mean_shift_lsh_model
import org.apache.spark.mllib.clustering.msLsh.local.Fcts
/**
 * @author Beck Gaël
  ...
  def run(sc: SparkContext, data: RDD[(Long, Vector)]) : ArrayBuffer[Mean_shift_lsh_model] = 
  {
    ...
    */
    ...
    val readyToLabelization = if( nbblocs2 == 1 ) lineageRDDs.last.map{ case (id, mod, originalVector, hashV, _) => (id, mod, originalVector)}.coalesce(1)
      ...
      .map{ case (id, mod, originalVector, hashV, _) => (id, mod, originalVector)}
    ...
    val labelizing = () => 
    {
      ...
      */
      ...
      while( ! clusterIDsOfSmallerOne.isEmpty )
      {
        ...
        val sortedClosestCentroid = toGatherCentroids.map{ case (id, newClusterID, vector, cardinality, _) => (id, vector, Vectors.sqdist(vector, origVector),
          newClusterID, cardinality) }.sortBy{ case (_, _, dist, _, _) => dist }
        ...
        while( totSize <= cmin )
        {
          val (idxK, vectorK, _, clusterIDK, cardinalityK) = lookForNN.map(v => toGatherCentroids.map{ case (id, newClusterID, vector, cardinality, _) =>
            (id, vector, Vectors.sqdist(vector, origVector), newClusterID, cardinality) }.filter{ case (_, _, _, newClusterID, _) =>
            ! oldClusterIDs.contains(newClusterID) }.sortBy{ case (_, _, dist, _, _) => dist }.head).sortBy{ case (_, _, dist, _, _) => dist }.head
          lookForNN += vectorK
