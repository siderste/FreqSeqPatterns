package org.apache.spark.mllib.clustering.patchwork

/**
  * clustering.patchwork.PatchWork is a novel density-grid clustering algorithm for Apache Spark.
  * It has linear complexity and near linear horizontal scalability.
  * As a result, clustering.patchwork.PatchWork can cluster a billion points in a few minutes only.
  *
...
import PatchWorkUtils.{DataPoint, Epsilon}
...
object PatchWorkUtils {
  type DataPoint = ( Int, Array[Double] )// sider
...
class PatchWorkCell(val cellNumber: Array[Int]) extends Serializable {
  ...
  def this(p: DataPoint, eps: Epsilon) {
    this(p._2.zipWithIndex.map(x => math.ceil(x._1 / eps(x._2)).toInt))
  }
...
class PatchWorkModel(private var epsilon: Epsilon,
  ...
  def predict(point: DataPoint): PatchWorkCluster = {
    val cellKey = new PatchWorkCellKey(point._2.zipWithIndex.map(x => math.ceil(x._1 / this.epsilon(x._2)).toInt))
  ...
  /**
    ...
    * cell belongs to.
    *
    * @param cell_id A cell
    * @return The cluster the cell belongs to
    */
  def predict(cell_id: String): PatchWorkCluster = {
    val cellKey = new PatchWorkCellKey( cell_id.split(",").map(x=>x.toInt) )
    val cl = this.clusters.filter(cluster => cluster.cellsList.contains(cellKey))
    if (cl.isEmpty) {
      // point is noise
      val cluster = new PatchWorkCluster(-1)
...
/**
  * clustering.patchwork.PatchWork grid-density clustering algorithm
  *
  ...
  */
class PatchWork(var epsilon: Epsilon, var minPts: Int, var ratio: Double, var minCell: Int) extends Serializable {

  def setParams(epsilon: Array[Double], minPts: Int, ratio: Double, minCell: Int) = {
  ...
  def run(data: RDD[DataPoint]): PatchWorkModel = {
    if (data.getStorageLevel == StorageLevel.NONE) {
      ...
      // println("The input data is not cached, which may impact performance.")
    }
  ...
  private def runAlgorithm(data: RDD[DataPoint]): PatchWorkModel = {
    ...
    data.collect().foreach{p=>
      println("_1="+p._1+"_2="+p._2.mkString(","))
      (p._2.zipWithIndex.map{x =>
        math.ceil(x._1 / epsilon(x._2)).toInt}
        .mkString(","), 1)
    }
    */
    ...
    val cardinalsPatchwork: RDD[(String, Int)] = data.map { p =>
      (p._2.zipWithIndex.map{x =>
        // TODO: use another assignment to cells
        math.ceil(x._1 / epsilon(x._2)).toInt}
        .mkString(","), 1)
    }.reduceByKey(_ + _)
    ...
    val clusters = createClusters(cardinalsPatchwork.map(x =>
      (x._1.split(",").map(_.toInt), x._2) // (Array[Int], Int)
    ).collect().toList, minPts) // List[(Array[Int], Int)], minPts

    // returning the clustering.patchwork.PatchWorkModel
    new PatchWorkModel(epsilon, cardinalsPatchwork, clusters)
  }

  /**
    * Runs the algorithm on the input cells rdd to build the cluster.
    *
    * @param data Input cells rdd (preprocessed)=> RDD[(cellId, cellCardinal)]
    * @return The model
    */
  def runAlgorithmOnCells(data: RDD[(String, Int)]): PatchWorkModel = {
    // creates clusters from the set of cells got from last operation
    val clusters = createClusters(data.map(x =>
      (x._1.split(",").map(_.toInt), x._2) // (Array[Int], Int)
    ).collect().toList, minPts) // List[(Array[Int], Int)], minPts

    // returning the clustering.patchwork.PatchWorkModel
    new PatchWorkModel(epsilon, data, clusters)
  }
  ...
  /**
    * To deal with any dimensionality and return ALL near by cells (plus the given)
    * Sideridis
    * @param p
    * @return
    */
  def allNearCells(p: Array[Int]): List[Array[Int]] = {

    if (p.length == 3) {

      val tailList = getNearCell(p.tail)

      List.concat(
        tailList.map(x => Array.concat(Array(p.head), x)),
        tailList.map(x => Array.concat(Array(p.head - 1), x)),
        tailList.map(x => Array.concat(Array(p.head + 1), x))
      )
    }else{

      val tailList = allNearCells(p.tail)
  ...
  def createClusters(cardinalsPatchwork: List[(Array[Int], Int)], minPts: Int): List[PatchWorkCluster] = {
    ...
    cardinalsPatchwork.sortBy(_._2).foreach { cell =>
      ...
      if (cell._2 >= minPts) {
        ...
        } // else // already in a cluster then leave it here ??
      }
    }
