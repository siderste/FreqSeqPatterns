...
/**
  ...
  */
class MsLsh (var k: Int, var epsilon1: Double, var epsilon2: Double, var epsilon3: Double, var ratioToStop: Double, var yStarIter: Int,
             var cmin: Int, var normalisation: Boolean, var w: Double, var nbseg: Int, var nbblocs1: Int, var nbblocs2: Int,
             var nbLabelIter: Int) extends Serializable {
  ...
  def run(sc: SparkContext, data: RDD[(String, Vector)]) : ArrayBuffer[Mean_shift_lsh_model] =
  {
    ...
    data.unpersist(true)
    val lineageRDDs = ArrayBuffer.empty[RDD[(String, Vector, Vector, Double, Boolean)]]
    lineageRDDs += rdd_LSH
    ...
    if( ratioToStop == 1.0 ) // stop depends only on yStarIter nomatter of how many unconverged item exist
    {
      for( ind2 <- 0 until yStarIter )
      {
        // Increase convergence time
        //hashTab = Fcts.tabHash(nbseg, dim)
        //fix: set ind2 instead of ind so loop has a meaning....
        val rdd_LSH_ord =  lineageRDDs(ind2).sortBy({ case (_, _, _, hashValue, _) => hashValue}, ascending=true, nbblocs1).mapPartitions(it => {
          val approxKNN = it.toArray
          approxKNN.map{ case (id, originalVector, mod, hashV, stop) => {
            // TODO: add another distance but Vectors.sqdist
            val distKNNFromCurrentPoint = approxKNN.map{ case (id2, originalVector2, mod2, hashV2, _) =>
              //(originalVector2, MyUtils.jaccardDistance(mod, originalVector2))
              (originalVector2, Vectors.sqdist(mod, originalVector2))
            }.sortBy(_._2)
            val newMod = Fcts.computeCentroid(distKNNFromCurrentPoint.take(k), k)
            (id, originalVector, newMod, stop)
          }}.toIterator
        })
        lineageRDDs += rdd_LSH_ord.map{ case (id, originalVector, mod, stop) =>
          (id, originalVector, mod, Fcts.hashfunc(mod, w, b, hashTab), stop) }
      }
    }
    else
    {
      while( ind < yStarIter && ! stopIter )
      {
        ...
        val rdd_LSH_ord =  lineageRDDs(ind).sortBy({ case (_, _, _, hashValue, _) => hashValue}, ascending=true, nbblocs1).mapPartitions(it => {
          ...
          approxKNN.map{ case (id, originalVector, mod, hashV, _) => {
            ...
            val newMod = Fcts.computeCentroid(distKNNFromCurrentPoint.take(k), k)
            // TODO: add another distance but Vectors.sqdist
            //val stop = MyUtils.jaccardDistance(mod, newMod) <= epsilon1
            val stop = Vectors.sqdist(mod, newMod) <= epsilon1
            (id, originalVector, newMod, stop)
          }}.toIterator
        })
        lineageRDDs += rdd_LSH_ord.map{ case (id, originalVector, mod, stop) =>
          (id, originalVector, mod, Fcts.hashfunc(mod, w, b, hashTab), stop) }.cache
    ...
    val readyToLabelization =
      if( nbblocs2 == 1 ) lineageRDDs.last.map{ case (id, originalVector, mod, hashV, _) =>
        (id, originalVector, mod)}.coalesce(1)
      else lineageRDDs.last.sortBy({ case (_, _, _, hashValue, _) => hashValue}, ascending=true, nbblocs2)
        .map{ case (id, originalVector, mod, hashV, _) => (id, originalVector, mod)}
    ...
    val labelizing = () =>
    {
      val clusterByLshBucket = readyToLabelization.mapPartitionsWithIndex( (ind, it) => {
        val labeledData = ListBuffer.empty[(Int, (String, Vector, Vector))]
        val bucket = it.toBuffer
        var vec1 = bucket(Random.nextInt(bucket.size))
        var mod1 = vec1._3 // uses modes of bucket to form clusters inside each bucket-partition
        var clusterID = (ind + 1) * 10000
        while ( bucket.size != 0 )
        {
          val closestCentroids = bucket.filter{ case (id, originalVector, mod) => {
            // TODO: add another distance but Vectors.sqdist
            //MyUtils.jaccardDistance(mod, mod1) <= epsilon2
            Vectors.sqdist(mod, mod1) <= epsilon2
            } }
          val closestCentroids2 = closestCentroids.map{ case (id, originalVector, mod) => (clusterID, (id, originalVector, mod))}
          labeledData ++= closestCentroids2
          bucket --= closestCentroids
          if( bucket.size != 0 ) mod1 = bucket(Random.nextInt(bucket.size))._3
          clusterID += 1
        }
      ...
      /**
        ...
        */
      val rdd_Ystar_labeled = clusterByLshBucket // .map{ case (clusterID, (id, originalVector, mod)) => (clusterID, (id, mod, originalVector)) }
        .partitionBy(new HashPartitioner(dp))
        //.cache

      val centroidMapOrig = rdd_Ystar_labeled.map{ case (clusterID, (_, originalVector, mod)) =>
        (clusterID , originalVector.toArray) }.reduceByKeyLocally(_ + _) // sums per vectors dimensions...costly??
      ...
      while ( ! centroids.isEmpty ) {
        val closestClusters = centroids.filter{ case (clusterID, vector, clusterCardinality) => {
          // TODO: add another distance but Vectors.sqdist
          //MyUtils.jaccardDistance(vector, randomCentroidVector) <= epsilon3 }} //also keeps its self centroid
          Vectors.sqdist(vector, randomCentroidVector) <= epsilon3 }} //also keeps its self centroid
        // We compute the mean of the cluster
        val gatheredCluster = closestClusters.map{ case (clusterID, vector, clusterCardinality) =>
          (vector.toArray, clusterCardinality)
      ...
      val toGatherCentroids = newCentroids.zipWithIndex.map{ case ((clusterID, vector), id) =>
        (id, clusterID, vector, numElemByCluster(clusterID), clusterID)}//.par
      ...
      while( ! clusterIDsOfSmallerOne.isEmpty )
      {
        ...
        val sortedClosestCentroid = toGatherCentroids.map{ case (id, newClusterID, vector, cardinality, _) =>
          // TODO: add another distance but Vectors.sqdist
          //(id, vector, MyUtils.jaccardDistance(vector, origVector), newClusterID, cardinality)
          (id, vector, Vectors.sqdist(vector, origVector), newClusterID, cardinality)
        }.sortBy{ case (_, _, dist, _, _) => dist }
        ...
        while( totSize <= cmin )
        {
          val (idxK, vectorK, _, clusterIDK, cardinalityK) = lookForNN.map(v => toGatherCentroids.map{
            case (id, newClusterID, vector, cardinality, _) =>
              // TODO: add another distance but Vectors.sqdist
              //(id, vector, MyUtils.jaccardDistance(vector, origVector), newClusterID, cardinality)
              (id, vector, Vectors.sqdist(vector, origVector), newClusterID, cardinality)
          }.filter{ case (_, _, _, newClusterID, _) => ! oldClusterIDs.contains(newClusterID) }
            .sortBy{ case (_, _, dist, _, _) => dist }.head).sortBy{ case (_, _, dist, _, _) => dist }.head
          lookForNN += vectorK
        ...
        idxToReplace ++= toGatherCentroids.filter{ case (_, newClusterID, _, _, _) =>
          oldClusterIDs.contains(newClusterID) }.map{ case (id, _, _, _, _) => id }
      ...
      val newCentroidIDByOldOneMap = toGatherCentroids.map{ case (id, newClusterID, vector, cardinality, originalClusterID) =>
        (originalClusterID, newClusterID) }.toMap

      val newCentroidIDByOldOneMapBC = oldToNewLabelMap.map{ case (k, v) => (k, newCentroidIDByOldOneMap(v)) }

      val partitionedRDDF = rdd_Ystar_labeled.map{ case (clusterID, (id, originalVector, mod)) =>
        (newCentroidIDByOldOneMapBC(clusterID), (id, originalVector)) }.partitionBy(new HashPartitioner(dp))
...
object MsLsh {
  ...
  def train(sc:SparkContext, data:RDD[(String,Vector)], k:Int, epsilon1:Double, epsilon2:Double, epsilon3:Double,
            ratioToStop:Double, yStarIter:Int, cmin:Int, normalisation:Boolean, w:Double, nbseg:Int, nbblocs1:Int,
            nbblocs2:Int, nbLabelIter:Int) : ArrayBuffer[Mean_shift_lsh_model] =
    new MsLsh(k, epsilon1, epsilon2, epsilon3, ratioToStop, yStarIter, cmin, normalisation, w, nbseg, nbblocs1,
      nbblocs2, nbLabelIter).run(sc, data)

  /**
    * Restore RDD original value
    */
  val descaleRDD = (toDescaleRDD:RDD[(Int, (String, Vector))], maxMinArray:(Array[Double], Array[Double])) =>
  {
  ...
  /**
    ...
    */
  def imageAnalysis(msmodel:Mean_shift_lsh_model) : RDD[(String, Vector, Int)] =
    descaleRDD(msmodel.labelizedRDD, msmodel.maxMinArray).map{ case (clusterID, id, _) =>
      (id, msmodel.clustersCenter(clusterID), clusterID) }
  ...
  def saveImageAnalysis(msmodel:Mean_shift_lsh_model, path:String, numpart:Int=1) : Unit =
    descaleRDD(msmodel.labelizedRDD, msmodel.maxMinArray).map{ case (clusterID, id, vector) =>
      (id, msmodel.clustersCenter(clusterID), clusterID) }.sortBy(_._1, ascending=true, numpart).saveAsTextFile(path)

  /**
    * Get an RDD[ID,cluster_Number]
    */
  def getlabeling(msmodel:Mean_shift_lsh_model) : RDD[(String, Int)] = msmodel.labelizedRDD.map{ case (clusterID, (id, _)) => (id, clusterID) }
   ...
   */
  def prediction(v:Vector, mapCentroid:Map[Int,Vector]) : Int = mapCentroid.map{ case (clusterID, centroid) =>
    // TODO: add another distance but Vectors.sqdist
    //(clusterID, RV.vectorsDistance(v, centroid)) }.toArray.sortBy(_._2).head._1
    (clusterID, Vectors.sqdist(v, centroid)) }.toArray.sortBy(_._2).head._1
}