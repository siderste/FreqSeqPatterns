...
import org.apache.spark.mllib.clustering.LocalKMeans.{logInfo, logWarning}
...
object MyLocalKMeans extends Logging{
  ...
  def kMeansPlusPlus(
    ...
    val costArray = points.map(RVDistanceMeasure.fastDistance(_, centers(0)))
    ...
    for (i <- 1 until k) {
      ...
      for (p <- points.indices) {
        costArray(p) = math.min(
          RVDistanceMeasure.fastDistance(points(p), centers(i)),
          costArray(p))
      }

    }

    val distanceMeasureInstance = new RVDistanceMeasure
