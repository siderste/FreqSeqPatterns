...
import org.apache.spark.mllib.clustering.dbscan.irvingc
import org.apache.spark.mllib.clustering.dbscan.irvingc.DBSCANLabeledPoint._
...
class DBSCAN private (
  ...
  private def train(vectors: RDD[(String, Vector)]): DBSCAN = {
  ...
  private def toMinimumBoundingRectangle(vector: (String, Vector) ): DBSCANRectangle = {
    val point = irvingc.DBSCANPoint(vector)
